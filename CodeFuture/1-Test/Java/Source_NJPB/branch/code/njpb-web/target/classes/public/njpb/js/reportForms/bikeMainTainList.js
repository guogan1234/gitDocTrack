...
function initSelect() {
    ...
    $.ajax({
        type: 'GET',
        url: '/business-resource/estates/findBikeByConditions',
        dataType: 'JSON',
        contentType: 'application/json',
        success: function (retData) {
            var data = [];
            ...
            if (retData.results.length > 0) {
                for (var i = 0; i < retData.results.length; i++) {
                    data.push({
                        ...
                        text: retData.results[i].bikeFrameNo,
                    });
                }
                $("#bikeFrameNo").select2({
                    data: data,
                ...
                $("#bikeFrameNo").val("-1").trigger("change");
            }
        },
    });
    ...
    $.ajax({
        type: 'GET',
        url: '/business-resource/users/findByRoleIdAndCorpId?roleId=' + 4,
        dataType: 'JSON',
        contentType: 'application/json',
        success: function (retData) {
            var data = [];
...
function doSearch() {
    ...
    if ($("#bikeFrameNo").val() != -1) {
        pageParam.bikeFrameNo = $("#bikeFrameNo").val();//状态
    } else {
...
function initDataTable() {
    ...
    var workOrder = {
        getQueryCondition: function (data) {
            ...
            param.bikeFrameNo =  pageParam.bikeFrameNo;
    ...
    dataTable = $('#workOrderPrint').dataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            $.ajax({
                type: "GET",
                url: "/business-resource/bikeRepairList/findByConditions",
                cache: false,  //禁用缓存
                data: param,  //传入组装的参数
                dataType: "json",
                success: function (result) {
                    ...
                    $(dataTable.column(8).footer()).html(t);
                }
            });
        },
        ...
        initComplete: function () {
            var api = this.api();
            var fda = api.column(8).data().reduce(function (a, b) {
                    return (parseFloat(a) + parseFloat(b)).toFixed(7);
                }
            );
            var j = parseInt(fda);
            $(api.column(0).footer()).html("工单总数：");
            $(api.column(8).footer()).html(j);
        },
        ...
        "columnDefs": [

            {
                ...
                'render': function (data, type, row, meta) {
                    return null2black(row.bikeFrameNo);
                }
            },
            {
                'targets': 4,
            ...
            },
            {
                ...
                'render': function (data, type, row, meta) {
                    ...
                    return '<button id="btnEdit_' + row.id + '" json="' + JSON.stringify(row).replace(/"/g, '\\\'')
                        + '" data-toggle="modal" onclick="openDialogDetail(' + row.bikeFrameNo + ','+ row.projectId + ',' + row.stationId + ',' + row.estateTypeId + ',' + row.faultTypeId + ',' + row.statusId + ',' + row.repairEmployee + ',' + meta.row + ',' + row.id + ')" class="btn btn-warning" title="编辑" data-toggle="modal"><i class="glyphicon glyphicon-pencil"></i> </button> ';
                }
            }
...
function openDialogDetail(bikeFrameNo,projectId, stationId, estateTypeId, faultTypeId, statusId, repairEmployee, rowid, domId) {
    ...
    rowJson = json;
    console.log(rowJson)
    urlTemp = "/business-resource/workOrders/bikeList?1=1&typeId=2";
    ...
    if (bikeFrameNo != null) {
        urlTemp += "&bikeFrameNo=" + bikeFrameNo;
        bikeFrameNoTemp = bikeFrameNo;
    }
...
function exportDetail() {
    ...
    var url = '/business-resource/bikeDetailExport?1=1&typeId=2';
    ...
    if (bikeFrameNoTemp!=null) {
        url += '&bikeFrameNo=' + bikeFrameNoTemp;
    }
    if (isNotEmpty($("#beginTime").val())) {
