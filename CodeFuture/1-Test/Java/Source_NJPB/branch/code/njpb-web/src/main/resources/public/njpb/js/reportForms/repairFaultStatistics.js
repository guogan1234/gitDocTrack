...
function initSelect() {
    ...
    $("#workOrderType").val("-1").trigger("change");
    ...
    $("#reportWay").val("-1").trigger("change");
...
function doRepairSearch() {
    ...
    if ($("#workOrderType").val() != -1 ) {
        pageParam.typeId = $("#workOrderType").val();//工单类型
    }
    if ($("#reportWay").val() != -1) {
        pageParam.reportWay = $("#reportWay").val();//状态
    }
...
function initDataTable() {
    ...
    dataTable = $('#stationEstatesFault-list').dataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            $.ajax({
                type: "GET",
                url: "/business-resource/repairFaultStatistics/findByConditions",
                cache: false,  //禁用缓存
                data: param,  //传入组装的参数
                dataType: "json",
                success: function (result) {
...
function exportExcel() {
    ...
    //     return;
    // }
    var url = '/business-resource/repairFaultStatisticsExport?1=1&corpId='+pageParam.corpId;
